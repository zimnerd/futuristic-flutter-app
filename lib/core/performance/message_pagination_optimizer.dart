...
class MessagePaginationOptimizer {
  ...
  void addMessages(
    ...
    if (cachedMessages.length > ChatPerformanceConfig.maxCachedMessages) {
      final excess =
          cachedMessages.length - ChatPerformanceConfig.maxCachedMessages;
      cachedMessages.removeRange(0, excess);

      if (kDebugMode) {
        print(
          'MessageCache: Trimmed $excess old messages for conversation $conversationId',
        );
      }
    }
    ...
    ChatPerformanceMetrics.recordMessageLoad(
      conversationId,
  ...
  List<MessageModel> getMessagesRange(
    String conversationId,
    int start,
    int end,
  ) {
  ...
  Map<String, dynamic> getMemoryInfo() {
    ...
    return {
      ...
      'averageMessagesPerConversation': _messageCache.isEmpty
          ? 0
          : totalMessages / _messageCache.length,
      'memoryEfficiencyScore':
          totalMessages <= ChatPerformanceConfig.maxCachedMessages
          ? 'Good'
          : 'Needs Optimization',
    };
  }

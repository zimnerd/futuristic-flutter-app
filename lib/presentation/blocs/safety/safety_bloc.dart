...
class SafetyBloc extends Bloc<SafetyEvent, SafetyState> {
  ...
  Future<void> _onLoadSafetyData(
    LoadSafetyData event,
    ...
    try {
      ...
      final scoreData = futures[4] as Map<String, dynamic>?;
      ...
      emit(
        state.copyWith(
          ...
          safetyScore: scoreData?['safetyScore']?.toDouble(),
        ),
      );
    } catch (e) {
  ...
  Future<void> _onReportContent(
    ...
    try {
      final report = await _safetyService.reportContent(
        ...
        contentType: 'message', // Default to message, could be passed in event
        reportType: event.reportType,
        description: event.description,
      );
  ...
  Future<void> _onLoadSafetySettings(
    LoadSafetySettings event,
    Emitter<SafetyState> emit,
  ) async {
    emit(state.copyWith(isLoading: true));
  ...
  Future<void> _onUpdateSafetySettings(
    ...
    try {
      ...
      final newSettings = SafetySettings(
        ...
        checkInRemindersEnabled: event.incidentReporting,
      );
  ...
  Future<void> _onLoadSafetyScore(
    LoadSafetyScore event,
    Emitter<SafetyState> emit,
  ) async {
    emit(state.copyWith(isLoading: true));
  ...
  Future<void> _onLoadSafetyReports(
    LoadSafetyReports event,
    Emitter<SafetyState> emit,
  ) async {
    emit(state.copyWith(isLoading: true));
  ...
  Future<void> _onTriggerEmergencyContact(
    TriggerEmergencyContact event,
    Emitter<SafetyState> emit,
  ) async {
  ...
  Future<void> _onSubmitPhotoVerification(
    SubmitPhotoVerification event,
    Emitter<SafetyState> emit,
  ) async {
    emit(state.copyWith(isLoading: true));
  ...
  Future<void> _onSubmitIdVerification(
    SubmitIdVerification event,
    Emitter<SafetyState> emit,
  ) async {
    emit(state.copyWith(isLoading: true));
  ...
  Future<void> _onReportDateSafetyConcern(
    ReportDateSafetyConcern event,
    Emitter<SafetyState> emit,
  ) async {
    emit(state.copyWith(status: SafetyStatus.reporting, isLoading: true));
  ...
  Future<void> _onCheckUserSafety(
    CheckUserSafety event,
    Emitter<SafetyState> emit,
  ) async {
    emit(state.copyWith(isLoading: true));

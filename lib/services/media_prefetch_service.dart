...
class MediaPrefetchService {
  ...
  void updateConfig(PrefetchConfig config) {
    _config = config;
    AppLogger.info('MediaPrefetchService: Config updated - prefetchCount: ${config.prefetchCount}, cellular: ${config.enableOnCellular}');
  }
  ...
  void setMemoryPressure(bool pressure) {
    ...
    if (pressure) {
      AppLogger.warning('MediaPrefetchService: Memory pressure detected, pausing prefetch');
      _cancelAllPrefetch();
    }
  }
  ...
  Future<void> prefetchProfiles({
    ...
    if (_memoryPressure) {
      AppLogger.debug('MediaPrefetchService: Skipping prefetch due to memory pressure');
      return;
    }
    ...
    if (!shouldPrefetch) {
      AppLogger.debug('MediaPrefetchService: Skipping prefetch due to network conditions');
      return;
    }

    // Calculate range to prefetch (current + next N profiles)
    final startIndex = currentIndex;
    final endIndex = (currentIndex + _config.prefetchCount + 1).clamp(0, profiles.length);
    ...
    AppLogger.info('MediaPrefetchService: Prefetching ${profilesToPrefetch.length} profiles (index $startIndex to ${endIndex - 1})');

    // Prefetch each profile's images
    for (final profile in profilesToPrefetch) {
      if (_memoryPressure) {
        AppLogger.debug('MediaPrefetchService: Stopping prefetch due to memory pressure');
        break;
      }
  ...
  Future<void> _prefetchImage(String url) async {
    ...
    try {
      ...
      if (context != null) {
        ...
        AppLogger.debug('MediaPrefetchService: ✅ Prefetched image: ${_truncateUrl(url)}');
      } else {
        // If no context available, just trigger the image provider
        // which will still download and cache the image
        AppLogger.debug('MediaPrefetchService: ⚠️ No context for prefetch, skipping: ${_truncateUrl(url)}');
      }
    } catch (e) {
      // Silently fail for prefetch errors (non-critical)
      AppLogger.debug('MediaPrefetchService: ⚠️ Failed to prefetch ${_truncateUrl(url)}: $e');
    } finally {

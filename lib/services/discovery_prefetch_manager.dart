...
class DiscoveryPrefetchManager {
  static final DiscoveryPrefetchManager _instance = DiscoveryPrefetchManager._internal();
  ...
  // Configuration
  static const int _prefetchLimit = 10; // Prefetch 10 profiles for smooth browsing
  ...
  Future<List<UserProfile>> prefetchProfiles({
    ...
    try {
      AppLogger.info('Starting discovery profile prefetch (limit: $_prefetchLimit)');
      ...
      AppLogger.info('Discovery prefetch complete: ${profiles.length} profiles cached');
      
      return profiles;
    } catch (e, stackTrace) {
      AppLogger.error('Error prefetching discovery profiles: $e', e, stackTrace);

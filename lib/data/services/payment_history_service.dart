...
class PaymentHistoryService {
  ...
  // Streams
  Stream<List<PaymentTransaction>> get historyStream => _historyController.stream;
  ...
  Future<ApiResponse<List<PaymentTransaction>>> getPaymentHistory({
    ...
    try {
      ...
      if (filter != null) {
        ...
        if (filter.startDate != null) queryParams['start_date'] = filter.startDate!.toIso8601String();
        if (filter.endDate != null) queryParams['end_date'] = filter.endDate!.toIso8601String();
        if (filter.subscriptionId != null) queryParams['subscription_id'] = filter.subscriptionId;
        if (filter.minAmount != null) queryParams['min_amount'] = filter.minAmount;
        if (filter.maxAmount != null) queryParams['max_amount'] = filter.maxAmount;
      }

      final response = await _get('/payments/history', queryParams: queryParams);

      if (response['success'] == true) {
        final transactionsList = response['data'] as List;
        final transactions = transactionsList
            .map((json) => PaymentTransaction.fromJson(json as Map<String, dynamic>))
            .toList();
        ...
        return ApiResponse.error(response['error'] ?? 'Failed to fetch payment history');
      }
    } catch (e) {
  ...
  Future<ApiResponse<PaymentTransaction>> getTransactionById(String transactionId) async {
    try {
      ...
      final cachedTransactions = await _getCachedTransactions();
      final transaction = cachedTransactions.where((t) => t.id == transactionId).firstOrNull;
  ...
  Future<ApiResponse<String>> downloadReceipt(String transactionId) async {
    try {
      final response = await _get('/payments/transactions/$transactionId/receipt');

      if (response['success'] == true) {
        return ApiResponse.success(response['data']['receipt_url'] as String);
      } else {
        return ApiResponse.error(response['error'] ?? 'Failed to generate receipt');
      }
    } catch (e) {
  ...
  Future<ApiResponse<String>> exportPaymentHistory({
    ...
    try {
      ...
      if (filter != null) {
        ...
        if (filter.startDate != null) body['start_date'] = filter.startDate!.toIso8601String();
        if (filter.endDate != null) body['end_date'] = filter.endDate!.toIso8601String();
        if (filter.subscriptionId != null) body['subscription_id'] = filter.subscriptionId;
      ...
      if (response['success'] == true) {
        ...
        return ApiResponse.error(response['error'] ?? 'Failed to export payment history');
      }
    } catch (e) {
  ...
  Future<ApiResponse<PaymentStats>> getPaymentStats({
    ...
    try {
      ...
      if (response['success'] == true) {
        final stats = PaymentStats.fromJson(response['data'] as Map<String, dynamic>);
        return ApiResponse.success(stats);
      } else {
        return ApiResponse.error(response['error'] ?? 'Failed to fetch payment statistics');
      }
    } catch (e) {
      // Generate basic stats from cached data
      final cachedTransactions = await _getCachedTransactions();
      if (cachedTransactions.isNotEmpty) {
        final stats = _generateStatsFromCache(cachedTransactions, startDate, endDate);
        return ApiResponse.success(stats);
      }
    ...
    try {
      ...
      final results = cachedTransactions.where((transaction) {
        ...
        return transaction.description.toLowerCase().contains(queryLower) ||
               ...
               (transaction.subscriptionId?.toLowerCase().contains(queryLower) ?? false);
      }).toList();

      if (filter != null) {
        return ApiResponse.success(results.where((t) => filter.matches(t)).toList());
      }
  ...
  Future<void> _cacheTransactions(List<PaymentTransaction> transactions, bool clearFirst) async {
    try {
  ...
  PaymentStats _generateStatsFromCache(
    ...
    final completed = filtered.where((t) => t.status == PaymentTransactionStatus.completed);
    final totalAmount = completed.fold(0.0, (sum, t) => sum + t.amount);
    final refunded = filtered.where((t) => t.status == PaymentTransactionStatus.refunded);
    ...
    return PaymentStats(
      ...
      failedTransactions: filtered.where((t) => t.status == PaymentTransactionStatus.failed).length,
      averageTransactionAmount: completed.isNotEmpty ? totalAmount / completed.length : 0,

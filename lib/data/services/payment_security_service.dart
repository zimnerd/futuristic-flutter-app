...
class PaymentSecurityService {
  ...
  Future<SecurityValidationResult> validatePaymentSecurity({
    ...
    if (recentTransactions > settings.velocityLimit) {
      ...
      recommendations.add(
        'Multiple transactions detected. Consider adding delay',
      );
    }
    ...
    if (!isTrustedDevice) {
      ...
      recommendations.add(
        'New device detected. Consider additional verification',
      );
    }
  ...
  RiskLevel _calculateRiskLevel(List<SecurityIssue> issues) {
    ...
    final highRiskIssues = [
      SecurityIssue.suspiciousPattern,
      SecurityIssue.highVelocity,
    ];
  ...
  factory SecurityEvent.fromJson(Map<String, dynamic> json) {
    return SecurityEvent(
      ...
      issues:
          (json['issues'] as List?)
              ?.map(
                (e) => SecurityIssue.values.firstWhere(
                  (issue) => issue.name == e,
...
class Platform {
  static bool get isAndroid =>
      true; // Would be Platform.isAndroid in real implementation
  static bool get isIOS =>
      false; // Would be Platform.isIOS in real implementation
}
